...
def informforcls1(conditions, subjects):
    ...
    if subjects != []:
        report += "Onderwerp(en):\t"
        report += makelistreport(subjects)
    if conditions != set():
        report += "Conditie(s):\t"
        report += makelistreport(list(conditions))
    return report
...
def informforcls2and3and4(cls, conditions, dimensions, subjects):
    ...
    if subjects != set():
        if cls == 2:
            report += "Onderwerp:\taantal " + subjects[0].__repr__() + "\n"
        if cls == 3:
            report += "Onderwerp:\t"
    ...
    if dimensions != set():
        report += "Dimensie(s):\t"
        report += makelistreport(list(dimensions))
    if conditions != set():
        report += "Conditie(s):\t"
        report += makelistreport(list(conditions))
    return report
...
def informforcls5(conditions, dimensions, subjects, denominators):
    report = ""
    if subjects != []:
        report += "Onderwerp:\tgemiddeld aantal " + subjects[0].__repr__() + " per " + denominators.pop().__repr__() + "\n"
    if dimensions != set():
        report += "Dimensie(s):\t"
        report += makelistreport(list(dimensions))
    if conditions != set():
...
def informforcls6(conditions, dimensions, subjects, order):
    ...
    if subjects != [] and order != None:
        ...
        if order == "asc":
            ...
            report += "grootste 5 "
        if dimensions != set():
            report += d[0].var.domain.altname
        else:
            report += subjects[0].var.domain.altname
        report += " volgens "
        if subjects[0].var.name.startswith("een"):
            report += "aantal " + subjects[0].__repr__() + " van een " + d[0].var.domain.name
        else:
            report += subjects[0].__repr__()
        if conditions != set():
            report += "Conditie(s):\t"
...
def informforcls7(conditions, dimensions, subjects, order):
    ...
    d = list(dimensions)
    if subjects != [] and order != None:
        report += "Onderwerp:\tde 5 " + d[0].var.domain.altname
        report += " met het "
        if order == "asc":
            report += "kleinste aantal "
        else:
            report += "grootste aantal "
        report += subjects[0].var.domain.altname + "\n"
    if conditions != set():
        report += "Conditie(s):\t"
        report += makelistreport(list(conditions))
...
def informforcls10(conditions, subjects, order):
    report = ""
    if subjects != [] and order != None:
        report += "Onderwerp:\tde 5 " + subjects[1].var.domain.altname
        report += " met " + subjects[0].var.article
        if order == "asc":
            report += " laagste "
        else:
            report += " hoogste "
...
def informforcls11(conditions, dimensions, subjects, denominators, order):
    ...
    if subjects != [] and order != None:
        report += "Onderwerp:\tde 5 " + d[0].var.domain.altname
        report += " met gemiddeld het "
        if order == "asc":
            report += "laagste aantal "
        else:
            report += "hoogste aantal "
        report += subjects[0].__repr__() + " per " + denominators.pop().__repr__() + "\n"
    if conditions != set():
        report += "Conditie(s):\t"
        report += makelistreport(list(conditions))
    return report
